...
### Why the custom providers exist instead of gvfs

Measured end-to-end against real servers in 2026-07 and decided against: gvfs's FUSE
bridge drops readdirplus, so listing costs one round trip per entry (1408 entries:
1.4 s on a LAN, an extrapolated 3m17s at 120 ms RTT), and bulk transfer loses 5-6x on
high-latency links. Full numbers and the cases where gvfs *is* the right tool in
`docs/why-not-gvfs.md` — read it before proposing to replace `src/core/network/`.
